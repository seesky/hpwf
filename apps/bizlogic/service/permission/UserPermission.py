...
class UserPermission(object):
    ...
    def GrantUserPermissions(userInfo, userIds, grantPermissionItemIds):
        """
          批量授予用户操作权限
          Args:
        ...
        """
        returnValue = 0
        #sequenceIds = SequenceService.GetBatchSequence(None, userIds.count() * grantPermissionItemIds.count())
        for i in range(len(userIds)):
            for j in range(len(grantPermissionItemIds)):
                #UserPermission.Grant(userInfo, sequenceIds[i * grantPermissionItemIds.count() + j], userIds[i], grantPermissionItemIds[j])
                UserPermission.Grant(userInfo, None, userIds[i],
                                     grantPermissionItemIds[j])
    ...
    def RevokeUserPermissions(self, userIds, revokePermissionItemIds):
        ...
        """
        returnValue = 0
        if userIds and revokePermissionItemIds:
            for uid in userIds:
    ...
    def GetScopeOrganizeIdsByUserId(self, userId, permissionItemCode):
        ...
        returnValue = Pipermissionscope.objects.filter(
            Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIORGANIZE') & Q(
                permissionid=Pipermissionitem.objects.get(code=permissionItemCode).id)).values_list('targetid',
                                                                                                    flat=True)
        return returnValue
    ...
    def GetScopeUserIdsByUserId(self, userId, permissionItemCode):
        """
          ...
          Returns:
              returnValue(string[]): 用户主键数组
        """
        #returnValue = PermissionScopeService.GetUserIds(None, userId, permissionItemCode)
        returnValue = UserPermission.GetUserIds(None, userId, permissionItemCode)
    ...
    def GetScopeRoleIdsByUserId(self, userId, permissionItemCode):
        ...
        """
        returnValue = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIROLE') & Q(permissionid=Pipermissionitem.objects.get(code=permissionItemCode).id)).values_list('targetid', flat=True)
        return returnValue
    ...
    def GrantUserRoleScope(self, userId, permissionScopeItemCode, grantRoleIds):
        ...
        for id in grantRoleIds:
            r = UserPermission.GrantRole(self, userId, permissionScopeItemCode, id)
            if r:
                returnValue = returnValue + 1
        return returnValue
    ...
    def GetScopePermissionItemIdsByUserId(self, userId, permissionItemCode):
        ...
        """
        returnValue = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIPERMISSIONITEM') & Q(permissionid=Pipermissionitem.objects.get(code=permissionItemCode).id)).values_list('targetid', flat=True)
        return returnValue

    def GrantUserPermissionItemScope(userInfo, userId, permissionItemCode, grantPermissionItemIds):
        """
          授予用户某个权限域的操作权限授权范围
          Args:
              userId (string): 用户主键
        ...
        for id in grantPermissionItemIds:
            UserPermission.GrantPermissionItem(userInfo, userId, permissionItemCode, id)
            returnValue = returnValue + 1
        return returnValue
    ...
    def GetScopeModuleIdsByUserId(self, userId, permissionItemCode):
        ...
        """
        returnValue = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIMODULE') & Q(permissionid__in=Pipermissionitem.objects.filter(code=permissionItemCode).values_list('id', flat=True))).values_list('targetid', flat=True)
        return returnValue

    def GrantUserModuleScope(userInfo, userId, permissionScopeItemCode, grantModuleIds):
        """
          授予用户某个权限域的模块授权范围
          Args:
              userId (string): 用户主键
        ...
        for id in grantModuleIds:
            UserPermission.GrantModule(userInfo, userId, permissionScopeItemCode, id)
            returnValue = returnValue + 1
        return returnValue
    ...
    def Grant(userInfo, sequence, userId, permissionItemId):
        """
          为了提高授权的运行速度
          Args:
              id (string): 主键
        ...
        resourcePermissionEntity.id = uuid.uuid4()
        ...
        resourcePermissionEntity.createon = datetime.datetime.now()
        resourcePermissionEntity.createby = userInfo.RealName
        resourcePermissionEntity.createuserid = userInfo.Id
        resourcePermissionEntity.modifiedon = resourcePermissionEntity.createon
        resourcePermissionEntity.modifiedby = userInfo.RealName
        resourcePermissionEntity.modifieduserid = userInfo.Id
    ...
    def Revoke(self, userId, permissionItemId):
        """
          ...
          Returns:
              returnValue(string): 主键
        """
        returnValue,v = Pipermission.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(permissionid=permissionItemId)).delete()
        return returnValue
    ...
    def RevokeOrganize(self, userId, permissionItemCode, revokeOrganizeId):
        """
          ...
          Returns:
              returnValue(string): 主键
        """
        returnValue,v = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIORGANIZE') & Q(targetid=revokeOrganizeId) & Q(permissionid=Pipermissionitem.objects.get(code=permissionItemCode).id)).delete()
        return returnValue
    ...
    def RevokeUser(self, userId, permissionItemCode, revokeUserId):
        """
          ...
          Returns:
              returnValue(string): 主键
        """
        returnValue = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIUSER') & Q(targetid=revokeUserId) & Q(permissionid=Pipermissionitem.objects.get(code=permissionItemCode).id)).delete()
        return returnValue
    ...
    def RevokeRole(self, userId, permissionItemCode, revokeRoleId):
        """
          ...
          Returns:
              returnValue(string): 主键
        """
        returnValue,v = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIROLE') & Q(targetid=revokeRoleId) & Q(permissionid=Pipermissionitem.objects.get(code=permissionItemCode).id)).delete()
        return returnValue
    ...
    def RevokePermissionItem(self, userId, permissionItemCode, revokePermissionId):
        """
          ...
          Returns:
              returnValue(string): 主键
        """
        returnValue,v = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIPERMISSIONITEM') & Q(targetid=revokePermissionId) & Q(permissionid=Pipermissionitem.objects.get(code=permissionItemCode).id)).delete()
        return returnValue

    def GrantModule(userInfo, userId, permissionItemCode, grantModuleId):
        """
          为了提高授权的运行速度
          Args:
              userId (string): 员工主键
    ...
    def RevokeModule(self, userId, permissionItemCode, revokeModuleId):
        """
          ...
          Returns:
              returnValue(string): 主键
        """
        returnValue = Pipermissionscope.objects.filter(Q(resourcecategory='PIUSER') & Q(resourceid=userId) & Q(targetcategory='PIMODULE') & Q(targetid=revokeModuleId) & Q(permissionid__in=
            Pipermissionitem.objects.filter(code=permissionItemCode).values_list('id', flat=True))).delete()
        return returnValue
